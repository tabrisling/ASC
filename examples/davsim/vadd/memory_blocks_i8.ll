; Minimal dav i8 memory block/gather LLVM IR smoke test for tools/asc-sim.
;
; This mirrors OpenTileAS simulator/cases/a5_memory_blocks_i8: GM data is
; copied to UB at byte offset 32, block offsets are copied as i32 values,
; vsldb materializes the contiguous 256B payload, vgatherb materializes eight
; 32B blocks, and both halves are copied back to GM.

define dso_local ptc_kernel void @dav_memory_blocks_i8(ptr addrspace(1) %data,
...
entry:
  ...
  %mask = call <256 x i1> @llvm.hivm.pset.b8(i32 0)
  %idx = call <64 x i32> @llvm.hivm.vldsx1.v64s32(ptr addrspace(6) inttoptr (i64 512 to ptr addrspace(6)), i32 0, i32 0, i32 0)
  %loaded = call <256 x i8> @llvm.hivm.vsldb(ptr addrspace(6) null, i32 65537, i32 0, <256 x i1> %mask)
  %gathered = call <256 x i8> @llvm.hivm.vgatherb.v310.v256s8(ptr addrspace(6) inttoptr (i64 32 to ptr addrspace(6)), <64 x i32> %idx, <256 x i1> %mask)
  call void @llvm.hivm.vstsx1.v256s8(<256 x i8> %loaded, ptr addrspace(6) inttoptr (i64 1024 to ptr addrspace(6)), i32 0, i32 2, i32 0, <256 x i1> %mask)
  call void @llvm.hivm.vstsx1.v256s8(<256 x i8> %gathered, ptr addrspace(6) inttoptr (i64 1280 to ptr addrspace(6)), i32 0, i32 2, i32 0, <256 x i1> %mask)
...
declare <256 x i8> @llvm.hivm.vsldb(ptr addrspace(6), i32, i32, <256 x i1>)
declare <256 x i8> @llvm.hivm.vgatherb.v310.v256s8(ptr addrspace(6), <64 x i32>, <256 x i1>)
...
!1 = !{ptr @dav_memory_blocks_i8, !"kernel", i32 1}
