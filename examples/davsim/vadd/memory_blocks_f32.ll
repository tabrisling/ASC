; Minimal dav f32 memory block/gather LLVM IR smoke test for tools/asc-sim.
;
; This mirrors OpenTileAS simulator/cases/a5_memory_blocks. The byte payload is
; exposed as float lanes by vsldb/vgatherb, but the final oracle is byte-for-byte
; raw data copied back from UB.

define dso_local ptc_kernel void @dav_memory_blocks_f32(ptr addrspace(1) %data,
...
entry:
  ...
  %loaded = call <64 x float> @llvm.hivm.vsldb(ptr addrspace(6) null, i32 65537, i32 0, <256 x i1> %mask)
  %gathered = call <64 x float> @llvm.hivm.vgatherb.v310.v64f32(ptr addrspace(6) inttoptr (i64 32 to ptr addrspace(6)), <64 x i32> %idx, <256 x i1> %mask)
  call void @llvm.hivm.vstsx1.v64f32(<64 x float> %loaded, ptr addrspace(6) inttoptr (i64 1024 to ptr addrspace(6)), i32 0, i32 2, i32 0, <256 x i1> %mask)
  call void @llvm.hivm.vstsx1.v64f32(<64 x float> %gathered, ptr addrspace(6) inttoptr (i64 1280 to ptr addrspace(6)), i32 0, i32 2, i32 0, <256 x i1> %mask)
...
declare <64 x float> @llvm.hivm.vsldb(ptr addrspace(6), i32, i32, <256 x i1>)
declare <64 x float> @llvm.hivm.vgatherb.v310.v64f32(ptr addrspace(6), <64 x i32>, <256 x i1>)
...
!1 = !{ptr @dav_memory_blocks_f32, !"kernel", i32 1}
